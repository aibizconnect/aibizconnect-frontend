...
export async function sendEmail(tenantId: string, msg: { to: string; subject: string; html: string; text?: string; footer?: "setup" | "appointment" | "none"; headers?: Record<string, string>; from?: { name?: string; email?: string }; actingUserKey?: string | null }): Promise<{ ok: boolean; id?: string; error?: string }> {
  ...
  // Per-user / workspace branding (D-404, RULING D-397). `actingUserKey` is opt-in, so existing
  // callers that don't pass it are byte-for-byte unchanged. When present: undefined → no wrap;
  // null/"" → workspace branding; a uid/email → that member's branding (member over workspace,
  // honouring locks). We wrap the body (header→body→signature→social→footer) and adopt the
  // resolved "From" unless the caller passed an explicit override.
  let html = msg.html;
  let text = msg.text;
  let fromOverride = msg.from;
  if (msg.actingUserKey !== undefined) {
    try {
      const { resolveEmailBranding, hasBranding, brandedEmailHtml, brandedEmailText } = await import("./email-branding");
      const b = await resolveEmailBranding(tenantId, msg.actingUserKey || undefined);
      if (hasBranding(b)) {
        html = brandedEmailHtml(b, html);
        if (text) text = brandedEmailText(b, text);
        if (!fromOverride) fromOverride = { name: b.fromName, email: b.fromEmail };
      }
    } catch { /* branding is best-effort; fall back to the raw message */ }
  }
  ...
  const fromName = fromOverride?.name?.trim() || ready.identity.sender_name;
  const overrideEmail = fromOverride?.email?.trim().toLowerCase();
  ...
  try {
    const body: Record<string, unknown> = { from, to: msg.to, subject: msg.subject, html: html + footer };
