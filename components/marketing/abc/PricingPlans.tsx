...
export default function PricingPlans({ tiers = DEFAULT_TIERS }: { tiers?: Tier[] }) {
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5" style={{ marginTop: 40, alignItems: "start" }}>
        {TIERS.map((t) => (
          <div key={t.name} style={{ position: "relative", background: v("--surface-card"), borderRadius: v("--radius-xl"), padding: 20, border: t.highlight ? `2px solid ${v("--border-brand")}` : `1px solid ${v("--border-subtle")}`, boxShadow: t.highlight ? v("--shadow-lg") : v("--shadow-xs") }}>
            ...
            <div style={{ marginTop: 14, display: "flex", alignItems: "baseline", gap: 4 }}>
              {t.m === null
                ? <span style={{ fontFamily: v("--font-display"), fontSize: v("--text-2xl"), fontWeight: 700, color: v("--text-strong") }}>Call us</span>
