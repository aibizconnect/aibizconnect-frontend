...
const STEPS = ["Start", "Basics", "AI", "Subdomain", "Design", "Review"] as const;
...
export default function WebsiteWizard({ tenantId }: { tenantId: string }) {
  ...
  // Upfront enrichment: read the existing site, then PRE-FILL the wizard.
  const analyze = () => {
  ...
  // Append an audience suggestion to the comma-separated field (toggle off if present).
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        {/* STEP 1 — Basics (pre-filled by the analyze step when available) */}
        {step === 1 && (
        ...
        {/* STEP 0 — Start: existing presence + analyze-and-prefill */}
        {step === 0 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-stretch gap-2">
                ...
                <button type="button" onClick={analyze} disabled={analyzing || !existingUrl.trim()}
              ...
              {enrich && (
                <p className={`mt-2 rounded-lg px-3 py-2 text-xs ${enrich.found ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>
                  {enrich.notes}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <button onClick={() => setSocialLinks([...socialLinks, ""])} className="text-sm font-medium text-[#1e3a8a]">＋ Add another</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            <h2 className="text-lg font-semibold text-slate-900">Let AI draft your pages</h2>
            <p className="text-sm text-slate-500">
              We&apos;ll start lean — a <strong>Home + Contact</strong> page drafted from your answers — so you can
              review and refine before adding the rest. Everything is a <strong>draft</strong>; nothing is published
              until you say so, and you can add more AI pages anytime from the editor.
            </p>
            <label className="flex items-center gap-2 rounded-lg border border-slate-200 p-3 text-sm text-slate-700">
              ...
              Yes, use AI to draft my site content
            </label>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4">
              ...
              <div className="flex items-stretch gap-2">
                <div className="flex flex-1 items-center rounded-lg border border-slate-300 focus-within:border-[#1e3a8a] focus-within:ring-1 focus-within:ring-[#1e3a8a]">
                  <input className="flex-1 rounded-l-lg px-3 py-2 text-sm outline-none"
                    value={subdomainRaw}
                    onChange={(e) => { setSubdomainRaw(e.target.value); setCheck(null); }}
                    placeholder="ali-realty" />
                  <span className="select-none px-3 text-sm text-slate-400">.{SUBDOMAIN_BASE}</span>
                </div>
                <button onClick={runCheck} disabled={checking || normalized.length < 3}
            ...
            <p className="text-xs text-slate-400">
              We reserve this address now. Your site stays a private draft — the public DNS is only set up when you publish.
            </p>
          </div>
        )}

        {/* STEP 4 — Design / template family */}
        {step === 4 && (
          <div className="space-y-4">
            <h2 className="text-lg font-semibold text-slate-900">Pick a starting look</h2>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {TEMPLATE_FAMILIES.map((f) => {
                const ft = FAMILY_THEME[f];
                return (
                  <button key={f} onClick={() => { setTemplateFamily(f); setPrimaryColor(ft.primary); }}
                    className={`rounded-xl border p-3 text-left text-sm transition ${
                      templateFamily === f ? "border-[#1e3a8a] ring-1 ring-[#1e3a8a]" : "border-slate-200 hover:border-slate-300"}`}>
                    {/* palette swatches */}
            ...
            {/* live font + palette preview for the selected family */}
        ...
        {step === 5 && (
          <div className="space-y-4">
            ...
            <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">
              This creates a <strong>draft</strong> website. Nothing is published, charged, or sent.
            </p>
