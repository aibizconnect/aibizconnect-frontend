...
| 3 | **Dashboard** | widgets, opp value, funnel | `dashboard` | ✅ **stats + graphs shipped (D-326)** | 6 KPI cards (contacts, open pipeline $, won revenue, collected, bookings, rating) + chart.js graphs: pipeline-by-stage bar, revenue-collected line (6mo, from payments ledger), new-contacts bar (6mo), opp-status doughnut, lead-sources doughnut + recent activity. New `buildDashboard()` aggregator, no DDL. |
| 4 | **Conversations** | Inbox · Manual Actions · Snippets · Trigger Links | `conversations` | ✅ **shipped (D-297..301, D-326..332)** | Unified inbox: thread list + pane + composer, unread, ContactDetail tab. Channels: **SMS** (Twilio, live) + **Email** + **Messenger / Instagram DM / WhatsApp** (Meta webhook → inbox, Graph-API reply) + Webchat. Inbound webhooks signature-verified; lead-ads → Contacts. Meta channels need the platform Meta app + App Review to go live (docs/META-INTEGRATION.md). Defer: inbound email, Manual Actions/Snippets sub-tabs. |
...
| 18 | **Reputation** | Overview · Requests · Reviews · Listings · Widgets | `reputation` | ✅ **depth shipped (D-322..325)** | Reviews + collection link + star dist (D-238) + **Requests** (send review link to a contact via email/SMS, 1:1, DND/Unsubscribed-skipped, requests log — migration 0061) + **Widget** (public embeddable `/reviews/[id]/widget` iframe + copy-embed). Listings deferred "soon" (needs Google Business). |
