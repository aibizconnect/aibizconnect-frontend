-- 0025_funnels.sql
-- Funnel Builder schema. Funnel steps are real pages (reuse website_pages), so each
-- step inherits the whole editor / critic / publish stack. Idempotent. Interim-
-- permissive RLS to match the current custom-JWT model (service-role writes).
