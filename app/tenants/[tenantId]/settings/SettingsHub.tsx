...
// ── Email Services (D-346) — tenant-wide sender identity that drives ALL sending ──────
function EmailServicesSection({ tenantId, isAdmin }: { tenantId: string; isAdmin: boolean }) {
  const [v, setV] = useState<EmailSettingsView | null | undefined>(undefined);
  ...
  const load = useCallback(() => getEmailSettings(tenantId).then((r) => {
    setV(r);
    if (r) { setName(r.sender_name); setEmail(r.sender_email); setRecords(r.dns_records_required ?? []); }
  }).catch(() => setV(null)), [tenantId]);
  useEffect(() => { load(); }, [load]);

  async function save() {
    setBusy("save"); setMsg(null);
    const r = await saveEmailSettings(tenantId, { sender_name: name, sender_email: email, apiKey: apiKey.trim() || undefined });
    setBusy(null);
    if (!r.ok) { setMsg(r.message ?? "Could not save."); return; }
    setRecords(r.records ?? []); setApiKey(""); setMsg("Saved. Add the DNS records at your domain host, then click Verify.");
    load();
  }
  async function verify() {
    ...
    setMsg(r.ok ? "All records verified ✓ — you can now send email." : "Some records aren't live yet — DNS can take up to an hour to propagate.");
    load();
  }
  async function copy(t: string) { try { await navigator.clipboard.writeText(t); setMsg("Copied to clipboard."); } catch { /* */ } }

  if (v === undefined) return <div className="py-8 text-center text-sm text-slate-400">Loading…</div>;
  return (
    <div className="max-w-2xl space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-slate-700">Email sending identity</h2>
        {v && <StatusPill status={v.status} />}
      </div>
      <p className="text-sm text-slate-500">This is the verified <b>From</b> address your workspace sends with — campaigns, appointment emails and follow-ups all use it. Verify the domain once and you&apos;re live everywhere.</p>

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <label className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-600">Sender name</span>
          <input className={inp} disabled={!isAdmin} value={name} onChange={(e) => setName(e.target.value)} placeholder="AIBizConnect" /></label>
        <label className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-600">Sender email</span>
          <input className={inp} disabled={!isAdmin} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="hello@yourdomain.com" /></label>
      </div>
      <label className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-600">Resend API key {v?.hasSecret && <span className="text-emerald-600">(stored ✓ — leave blank to keep)</span>}</span>
        <input className={inp} type="password" disabled={!isAdmin} value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder={v?.hasSecret ? "•••••• stored" : "re_…"} /></label>

      <div className="flex gap-2">
        <button onClick={save} disabled={!isAdmin || !!busy} className="rounded-lg bg-[#1e3a8a] px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{busy === "save" ? "Saving…" : "Save"}</button>
        {records.length > 0 && <button onClick={verify} disabled={!!busy} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 disabled:opacity-50">{busy === "verify" ? "Checking…" : "Verify DNS"}</button>}
      </div>

      {records.length > 0 && (
        <div className="rounded-xl border border-slate-200">
          <div className="border-b border-slate-100 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-400">DNS records — add these at your domain host</div>
          <div className="divide-y divide-slate-100">
            {records.map((r, i) => (
              <div key={i} className="flex items-start justify-between gap-2 px-3 py-2 text-xs">
                <div className="min-w-0">
                  <div className="flex items-center gap-2"><span className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[10px] text-slate-600">{r.type}</span><span className="truncate font-mono text-slate-700">{r.name}</span><StatusPill status={r.status} /></div>
                  <div className="mt-0.5 truncate font-mono text-slate-400">{r.value}</div>
                </div>
                <button onClick={() => copy(r.value)} className="shrink-0 rounded border border-slate-300 px-2 py-0.5 text-[11px] text-slate-600 hover:bg-slate-50">Copy</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {!isAdmin && <p className="text-xs text-slate-400">View only — an admin must change the sending identity.</p>}
    </div>
  );
}

// ── Phone Numbers (D-346) — one place to see every SMS/WhatsApp number ─────────────
function PhoneNumbersSection({ tenantId, onManage }: { tenantId: string; onManage: () => void }) {
  const [twilio, setTwilio] = useState<TwilioSettingsView | null>(null);
  const [whatsapp, setWhatsapp] = useState<WhatsAppNumberView[]>([]);
  const [loaded, setLoaded] = useState(false);
  useEffect(() => {
    Promise.all([getTwilioSettings(tenantId).catch(() => null), getWhatsAppNumbers(tenantId).catch(() => [])])
      .then(([t, w]) => { setTwilio(t); setWhatsapp(w as WhatsAppNumberView[]); setLoaded(true); });
  }, [tenantId]);

  const hasTwilio = !!twilio && (twilio.from_number || twilio.messaging_service_sid || twilio.status === "connected");
  if (!loaded) return <div className="py-8 text-center text-sm text-slate-400">Loading…</div>;
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onManage} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">Manage in Integrations →</button>
      </div>
      <p className="text-sm text-slate-500">Every texting number your workspace uses for SMS and WhatsApp, in one place. Connect or edit them under Integrations.</p>

      <div>
        <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">SMS (Twilio)</div>
        {hasTwilio ? (
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="grid h-8 w-8 place-items-center rounded-full bg-[#F22F46] text-xs font-bold text-white">T</span>
                <div>
                  <div className="text-sm font-medium text-slate-800">{twilio!.from_number || twilio!.messaging_service_sid || "Twilio"}</div>
                  <div className="text-xs text-slate-400">{twilio!.messaging_service_sid ? "Messaging Service" : "From number"} · SMS</div>
                </div>
              </div>
              <StatusPill status={twilio!.status} />
            </div>
          </div>
        ) : (
          <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-400">No SMS number connected — add Twilio in Integrations.</p>
        )}
      </div>

      <div>
        <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">WhatsApp</div>
        {whatsapp.length > 0 ? (
          <div className="space-y-2">
            {whatsapp.map((w) => (
              <div key={w.id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-4">
                <div className="flex items-center gap-2">
                  <span className="grid h-8 w-8 place-items-center rounded-full bg-[#25D366] text-xs font-bold text-white">W</span>
                  <div>
                    <div className="text-sm font-medium text-slate-800">{w.label || w.phoneNumberId}</div>
                    <div className="text-xs text-slate-400">phone-number-id {w.phoneNumberId} · WhatsApp</div>
                  </div>
                </div>
          ...
          <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-400">No WhatsApp number connected — add one in Integrations.</p>
        )}
      </div>
    </div>
  );
