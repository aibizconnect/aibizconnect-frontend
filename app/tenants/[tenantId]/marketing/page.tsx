...
export default async function MarketingPage({ params }: { params: Promise<{ tenantId: string }> }) {
  ...
  const [campaigns, ready, sms] = await Promise.all([listCampaigns(tenantId), emailReady(tenantId), twilioReady(tenantId).catch(() => false)]);
  let tags: string[] = [];
  try {
  ...
  return <MarketingHub tenantId={tenantId} initialCampaigns={campaigns} status={{ emailReady: ready.ok, emailReason: ready.reason, smsReady: sms, tags }} />;
}
