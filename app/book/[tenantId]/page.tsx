 ...
 */
export default async function BookingIndexPage({ params }: { params: Promise<{ tenantId: string }> }) {
  ...
  return (
    <main style={vars} className="min-h-screen bg-slate-50 px-4 py-12">
      <div className="mx-auto max-w-xl">
        ...
        {cals.length === 0 ? (
          ...
          <div className="space-y-3">
            {cals.map((c) => (
              <Link key={c.id} href={`/book/${tenantId}/${c.slug}`}
